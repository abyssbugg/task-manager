...
// Types imported from domain/schema

// Zod Schemas imported from domain/schema
...
class TaskManagerServer {
  ...
  private async loadTasks() {
    try {
      const loaded = await loadTaskManagerFile(TASK_FILE_PATH);
      if (!loaded) return; // keep in-memory data
      this.data = loaded;
  ...
  private async saveTasks() {
    try {
      await saveTaskManagerFileAtomic(TASK_FILE_PATH, this.data);
    } catch (error) {
